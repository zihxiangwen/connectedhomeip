...
void BLEManagerImpl::HandleTXCharCCCDWrite(uint16_t conn_id, int indicationsEnabled, int notificationsEnabled)
{
...
bool BLEManagerImpl::CloseConnection(BLE_CONNECTION_OBJECT conId)
{
    ...
    err = MapBLEError(le_disconnect((uint8_t)conId));
    if (err != CHIP_NO_ERROR)
    {
...
bool BLEManagerImpl::SendIndication(BLE_CONNECTION_OBJECT conId, const ChipBleUUID * svcId, const ChipBleUUID * charId, PacketBufferHandle data)
{
    ...
    server_send_data((uint8_t)conId, bt_matter_adapter_service_id, BT_MATTER_ADAPTER_SERVICE_CHAR_NOTIFY_CCCD_INDEX-1, data->Start(), data->DataLength(), GATT_PDU_TYPE_NOTIFICATION);
...
int BLEManagerImpl::ble_callback_dispatcher(void *param, void *p_cb_data, int type, T_CHIP_BLEMGR_CALLBACK_TYPE callback_type)
{
    ...
    case CB_PROFILE_CALLBACK:
		blemgr->gatt_svr_chr_access(param, (T_SERVER_ID)type, (TBTCONFIG_CALLBACK_DATA *)p_cb_data);
        break;
