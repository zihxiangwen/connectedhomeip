...
uint8_t OTAUpdater::Begin(void)
{
    ...
    return ESP_OK;
#else
    ChipLogError(BDX, "%s, %d",__FUNCTION__,__LINE__);
    return 0;
#endif
}

uint8_t OTAUpdater::Write(const void * data, size_t length)
{
#if 0
...
#else
    otaUpdateImageLen += length;
    ChipLogError(BDX, "%s, %d mOTAUpdateImageLen=%d ",__FUNCTION__,__LINE__,otaUpdateImageLen);
    return 0;
#endif
}

uint8_t OTAUpdater::Abort(void)
{
#if 0
    if (otaUpdateInProgress == false)
    {
...
uint8_t OTAUpdater::Apply(uint32_t delayedActionTime)
{
#if 0
    if (otaUpdateInProgress == true)
    {
